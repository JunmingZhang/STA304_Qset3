...
## Model Specifics
We will be using a random intercept model to model the voters who will vote for Donald Trump. We will be using sex, age_group, race, hispan, education, state, and vote_trump to model the probability of voting for Donald Trump. The logistic regression model we are using is:
...
The confusion matrix is used for summarizing predicted result. The row represents the prediction by algorithm, while the column represents the actual collected data. From the table, it is obvious that 801 is about twice larger than 329. This means that there are more cases predicted correctly than falsely in this model. However, the accuracy is 0.6484. It is not high enough to evaluate this model.
